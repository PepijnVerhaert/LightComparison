...
public class MilazzoAlgorithm : LightAlgorithm
{
    ...
    public override void CalculateLight(int x, int y, int rangeLimit)
    {
        ...
        for (int i = 0; i < _colorMap.Count; i++)
        {
            for (int j = 0; j < _colorMap[i].Count; j++)
            {
                ...
                color = _grid[i][j] ? Color.white * .7f : Color.black;
    ...
    void Compute(uint octant, int oX, int oY, int rangeLimit, uint x, Slope top, Slope bottom)
    {
        ...
        for (; x <= (uint)rangeLimit; x++) // (x <= (uint)rangeLimit) == (rangeLimit < 0 || x <= rangeLimit)
        {
            ...
            else // top < 1
            {
                ...
                if (BlocksLight(x, topY, octant, oX, oY)) // if the tile blocks light (i.e. is a wall)...
                {
                    ...
                    if (top.GreaterOrEqual(topY * 2 + 1, x * 2) && !BlocksLight(x, topY + 1, octant, oX, oY)) topY++;
                }
                else // the tile doesn't block light
                {
                    ...
                    uint ax = x * 2; // center
                    if (BlocksLight(x + 1, topY + 1, octant, oX, oY)) ax++; // use bottom-right if the tile above and right is a wall
            ...
            else // bottom > 0
            {
                ...
                if (bottom.GreaterOrEqual(bottomY * 2 + 1, x * 2) && BlocksLight(x, bottomY, octant, oX, oY) &&
                   !BlocksLight(x, bottomY + 1, octant, oX, oY))
                {
                    bottomY++;
                }
            }
            ...
            for (uint y = topY; (int)y >= (int)bottomY; y--) // use a signed comparison because y can wrap around when decremented
            {
                ...
                {
                    bool isOpaque = BlocksLight(x, y, octant, oX, oY);
                    ...
                    if (isVisible) SetVisible(x, y, octant, oX, oY);
                    ...
                    if (x != rangeLimit) // but don't bother adjusting them if this is the last column anyway
                    {
                        if (isOpaque)
                        {
                            ...
                            {                  // so adjust the bottom vector upward and continue processing it in the next column
                                ...
                                if (BlocksLight(x, y + 1, octant, oX, oY)) nx--; // top left if the corner is not beveled
                                ...
                                {                       // created by adjusting the bottom is only valid if that's the case
                                    ...
                                    else Compute(octant, oX, oY, rangeLimit, x + 1, top, new Slope(ny, nx));
                                }
                        ...
                        else
                        {
                            if (wasOpaque > 0) // if we found a transition from opaque to clear, adjust the top vector downwards
                            {
                                ...
                                uint nx = x * 2, ny = y * 2 + 1; // the bottom of the opaque tile (oy*2-1) equals the top of this tile (y*2+1)
                                                                 // NOTE: if you're using full symmetry and want more expansive walls (recommended), comment out the next line
                                if (BlocksLight(x + 1, y + 1, octant, oX, oY)) nx++; // check the right of the opaque tile (y+1), not this one
    ...
    bool BlocksLight(uint x, uint y, uint octant, int oX, int oY)
    {
        uint nx = (uint)oX, ny = (uint)oY;
        switch (octant)
        {
        ...
        if (nx >= _map.mapSize || ny >= _map.mapSize) return true;

        return _map.map[(int)nx][(int)ny];
    }

    void SetVisible(uint x, uint y, uint octant, int oX, int oY)
    {
        uint nx = (uint)oX, ny = (uint)oY;
        switch (octant)
        {
        ...
        if (nx >= _map.mapSize || ny >= _map.mapSize) return;

        _grid[(int)nx][(int)ny] = true;
    }

    private int GetDistance(int x, int y)
    {
        //manhattan
        return Mathf.Abs(0 - x) + Mathf.Abs(0 - y);
    }
}
